...
- Do compute every count you display; never assert or hardcode one (G9 standing rule 5). Any total, badge or tally, unread counts, group sizes, column tallies, must be derived from the actual data at runtime, never a literal typed into markup. Distrust the enumerator too: a filter that silently skips hidden or collapsed rows still reports zero errors, so cross-check two ways.
- Do render any NEW collection view in the G9 Grid System by default (G9 standing rule 3): equal-size bordered cards side by side with rows stacking down, one consistent border color, visible sequence numbers when order matters, sharp corners, dense and full-width, no visible scrollbars, cards inheriting the host theme rather than forcing their own background. Never a bare unnumbered table or one long stacked column. The existing newspaper column layout is the user's explicit exception and stays as is; this governs new surfaces unless the user asks for something else.
